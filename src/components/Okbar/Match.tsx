  ...
  return (
    <div className={`match-item ${selected?"match-selected":""}`}>
      ...
      <p>{match.description[0]} {match.description.slice(1).map(arg => (
        <span key={match.name+"."+arg} className={`match-arg ${(autofillHighlight && (autofillIndex+1) == match.description.findIndex(a => a === arg)) ? "match-arg-selected" : ""}`} title={match.references[arg]}>{arg}</span>
      ))}</p>
    </div>
  );
}
